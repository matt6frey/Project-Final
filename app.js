...
app.post("/recipe-lookup", (req, res) => {
  ...
  const items = duplicateArray(req.body.items)
    .join(",")
    .toLowerCase(); // Join items into an http friendly str.
  ...
  checkDB(items).then(hasEntry => {
    if (!hasEntry) {
      ...
      unirest
        .get(
          `https://spoonacular-recipe-food-nutrition-v1.p.mashape.com/recipes/findByIngredients?ingredients=${items}&number=${number}&ranking=1`
        )
        .header(
          "X-Mashape-Key",
          "UmggyaDjvCmsh4jkCmZdRKKLMQ7Dp1oLVUDjsnb1e0yJuWBKSr"
        ...
        .end(function(result) {
          ...
          getRecipeDetails(
            ...
            detail => {
              ...
              knex("recipe_queries")
                .insert({ query_id: detail[keys[0]].queryID, query_str: items })
                .then(submitted => {
                  keys.forEach(key => {
                    knex("recipes")
                      .insert({
                        rid: detail[key].rid,
                        query_id: detail[key].queryID,
                        title: detail[key].title,
                        image: detail[key].image,
                        serves: detail[key].serves,
                        prep_time: detail[key].prepTime,
                        ingredients: detail[key].ingredients,
                        steps: detail[key].steps
                      })
                      .then(submitted => {
                        console.log(
                          `Added ${keys.length} recipe entries to database.`
                        );
                      });
                  });
                  res.status(200);
      ...
      knex("recipes")
        .where("query_id", hasEntry[0].query_id)
        .then(result => {
          console.log(
            `Found ${hasEntry.length} entries matching this query "${items}".`
          );
          console.log(result);
          res.status(200);
          res.json(result);
